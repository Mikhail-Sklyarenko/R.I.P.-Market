...
describe('ExtensionTradeAckService', () => {
  ...
  it('treats a different observed offer id as warn, not order mismatch', async () => {
    prisma.order.findUnique.mockResolvedValue({
      id: 'order-1',
      buyerId: 'buyer-1',
      sellerId: 'seller-1',
  ...
  it('ignores duplicate OFFER_SENT when a different offer is already linked', async () => {
    prisma.order.findUnique.mockResolvedValue({
      id: 'order-1',
      buyerId: 'buyer-1',
      sellerId: 'seller-1',
