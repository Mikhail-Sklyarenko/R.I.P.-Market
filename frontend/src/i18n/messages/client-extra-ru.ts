/**
 * Additive Russian message keys for components/pages that were wired to `t()`
 * ahead of the dictionary (staging previously showed raw key paths locally,
 * or fell back to Russian because the keys were entirely missing).
 *
 * Deep-merged into `ruMessages` in `ru.ts` via `deepMerge` — see
 * `../merge-messages.ts`. Keep this file additive only: prefer nesting under
 * an existing top-level namespace (account, catalog, wallet, ...) instead of
 * duplicating one.
 */
export const clientExtraRu = {
  ux: {
    savedTitle: "Избранное",
    savedBody: "Предметы сохранены для этого аккаунта в этом браузере. Цены Steam и площадки — ориентир; откройте предмет, чтобы проверить актуальные предложения.",
    savedOpen: "Посмотреть предложения",
    savedRemove: "Убрать из избранного",
    savedAdd: "Сохранить в избранное",
    savedEmpty: "Сохраняйте понравившиеся предметы закладкой в каталоге — так к ним проще вернуться.",
    savedError: "Не удалось сохранить. Проверьте доступность хранилища браузера; в избранном может быть не больше 100 предметов.",

    downloadUnavailable: "Загрузка архива недоступна. Обновите страницу или обратитесь в поддержку.",
    receiptCheckingTitle: "Получение подтверждено вами",
    receiptCheckingBody: "Сервис завершает проверку. Повторно принимать или отправлять обмен не нужно. Если ожидание затянулось, откройте обращение из этого заказа.",
    "completeSetup": "Подготовить аккаунт",
    "purchaseSaved": "Вернитесь к выбранному предмету",
    "purchaseSavedBody": "Завершите настройку и продолжите покупку. Наличие и цена предложения будут проверены заново.",
    "resumePurchase": "Продолжить покупку",
    "menuLabel": "Меню аккаунта",
    "steamGuideOnly": "Ориентир Steam · не цена предложения",
    "supportDetails": "Детали для поддержки",
    "installExtension": "Установка расширения",
    "extensionTitle": "Помощник для обменов в Steam",
    "extensionLead": "Подключите R.I.P Market в том же профиле браузера, где вы вошли в Steam. Помощник сверяет обмен и помогает отправить предмет; подтверждение Steam Guard и принятие остаются за вами.",
    "betaTitle": "Установка для закрытого тестирования",
    "betaBody": "Расширение пока устанавливается из архива, а не из магазина Chrome. Для теста нужен Chrome на компьютере. Если вы на телефоне, продолжите по этой ссылке на компьютере.",
    "download": "Скачать расширение",
    "stepDownload": "Скачайте и распакуйте архив",
    "stepDownloadBody": "Сохраните папку R.I.P-Market-Extension в постоянном месте. Не удаляйте её после установки.",
    "stepInstall": "Добавьте расширение в Chrome",
    "stepInstallBody": "Откройте chrome://extensions, включите «Режим разработчика», нажмите «Загрузить распакованное расширение» и выберите распакованную папку. При обновлении замените файлы в этой папке и нажмите кнопку перезагрузки расширения.",
    "stepConnect": "Подключите свой аккаунт",
    "stepConnectBody": "Вернитесь на эту страницу в том же профиле Chrome и нажмите «Подключить расширение». Для другого Steam-аккаунта используйте отдельный профиль браузера.",
    "permissionsTitle": "Зачем нужны разрешения",
    "permissionsBody": "Доступ к Steam нужен для чтения данных обмена и отправки выбранного предмета. Вкладки и выполнение сценариев используются для работы со страницей Steam; cookies — для сеанса Steam в браузере; хранилище — для связи с аккаунтом и очереди действий; уведомления — для важных шагов. Steam Guard и принятие обмена помощник не подтверждает за вас.",
    "resumeTrade": "Вернуться к сделке",
    "loginConnect": "Войти и подключить",
    "downloadVersion": "Версия",
    "extensionDisconnected": "Проверьте подключение перед обменом",
    "fundsDetails": "Суммы по сделкам",
    "fundsDetailsBody": "Откройте активные сделки, чтобы увидеть предмет, сумму и причину ожидания. По спорной сделке деньги остаются недоступны до решения.",
    "buyerSetupHint": "Steam и ссылка на обмен готовы. Для подсказок и сверки обмена подключите помощника. Ручное получение подтверждается отдельно на сайте."
},
  onboarding: {
    title: 'Как начать торговать',
    lead: 'Один аккаунт может и покупать, и продавать. Пройдите шаги ниже — порядок важен.',
    steamLabel: 'Привязать Steam',
    steamHint: 'Нужен для синхронизации инвентаря CS2 и участия в сделках.',
    steamAction: 'К привязке Steam',
    tradeUrlLabel: 'Указать Trade URL',
    tradeUrlHint: 'Без ссылки на обмен покупка и продажа недоступны.',
    tradeUrlAction: 'К Trade URL',
    extensionLabel: 'Подключить расширение',
    extensionHintOptional: 'Для автоотправки при продаже и подсказок при получении. Ручной обмен остаётся отдельным способом.',
    extensionHintInstall: 'Для работы помощника нужен Chrome на компьютере. Установите расширение по инструкции.',
    extensionAction: 'К расширению',
    readyText: 'Steam и ссылка на обмен настроены',
    readyExtensionOptional: '· помощник обмена ещё не подключён',
    catalogLink: 'Каталог',
    inventoryLink: 'Инвентарь',
    optionalSuffix: '(помощник обмена)',
  },
  catalog: {
    resetFilter: 'Сбросить',
    collapseFilter: 'Свернуть фильтр',
    expandFilter: 'Развернуть фильтр',
    popularBadge: 'Популярный',
  },
  dealFlow: {
    title: 'Как пройдёт сделка',
    steps: {
      reserve: {
        title: 'Резерв средств',
        description: 'Подтверждаете покупку — средства резервируются (hold) на кошельке.',
      },
      tradeOffer: {
        title: 'Обмен в Steam',
        description: 'Продавец отправляет trade offer в Steam.',
      },
      accept: {
        title: 'Принятие обмена',
        description: 'Вы принимаете предмет. Средства продавцу становятся доступны после проверки и применимого периода защиты.',
      },
      refund: {
        title: 'Возврат при сбое',
        description: 'Возврат зависит от состояния обмена. При споре деньги остаются в резерве до решения.',
      },
    },
  },
  buyRequestFlow: {
    steps: {
      request: {
        title: 'Оставляете заявку',
        description: 'Укажите максимальную цену или следите за любыми предложениями.',
      },
      notify: {
        title: 'Получаете уведомление',
        description: 'Когда появится подходящий лот — мы сразу сообщим в приложении.',
      },
      choose: {
        title: 'Выбираете лот',
        description: 'Смотрите float, стикеры и цену — и открываете конкретное предложение.',
      },
      buy: {
        title: 'Покупаете как обычно',
        description: 'Дальше стандартная покупка: резерв средств и обмен в Steam.',
      },
    },
  },
  errorAlert: {
    genericMessage: 'Что-то пошло не так. Попробуйте ещё раз.',
    networkMessage:
      'Нет связи с сервером R.I.P. Проверьте интернет / VPN и нажмите «Повторить».',
    retry: 'Повторить',
    codeLabel: 'Код: {{code}}',
    requestIdLabel: 'ID запроса: {{id}}',
  },
  escrow: {
    compact: 'Средства резервируются до подтверждения обмена в Steam.',
    full:
      'Деньги будут зарезервированы до подтверждения передачи предмета в Steam. После подтверждения средства переводятся продавцу.',
  },
  extension: {
    titleFull: 'Расширение для автообмена',
    titleCompact: 'Расширение',
    compactSubtitle: 'Помогает отправить предмет и проверить обмен. Guard подтверждаете вы.',
    installHow: 'Как установить',
    installBodyCompact:
      'Откройте страницу установки: скачайте архив, добавьте расширение в Chrome и подключите аккаунт.',
    installBodyFull:
      'Установите помощника по пошаговой инструкции, затем подключите его в этом профиле Chrome.',
    connectSuccess: 'Расширение подключено. Оно поможет отправить обмен; подтверждение Steam Guard остаётся за вами.',
    nextInventory: 'Дальше (~1 мин): откройте',
    nextInventoryLink: 'инвентарь CS2 в Steam',
    connected: 'Подключено',
    connectedUntil: 'Подключено до {{time}}',
    notConnected: 'Не подключено — автоотправка trade offer недоступна',
    browserHint: 'Используйте Chrome с тем же профилем, где открыт',
    browserHintSuffix: 'и вы залогинены под продавцом.',
    disconnect: 'Отключить',
    connecting: 'Подключаем…',
    connect: 'Подключить расширение',
    notInstalled: 'Установите расширение R.I.P Market и перезагрузите страницу.',
    pairFailedGeneric: 'Не удалось подключить расширение',
    pairSwGlitch:
      'Сбой расширения. Перезагрузите его на chrome://extensions (версия 0.6.39+) и нажмите «Подключить» снова.',
    pairNetworkError:
      'Нет связи с сервером R.I.P. Проверьте интернет / VPN и обновите страницу.',
    connectionError: 'Ошибка подключения',
    notFoundHint:
      'Не удалось связаться с расширением. Проверьте, что оно установлено и включено в этом профиле Chrome, затем повторите подключение.',
  },
  steamPriceAge: {
    today: 'сегодня',
    yesterday: 'вчера',
    daysAgo: '{{days}} дн. назад',
    oneWeekAgo: '1 нед. назад',
    weeksAgo: '{{weeks}} нед. назад',
  },
  item: {
    steamUpdated: 'Steam · обновлено {{age}}',
    priceMaybeStale: 'цена может быть устаревшей',
    bestOffer: 'Лучшее предложение',
    openBestOffer: 'Открыть лучшее предложение',
    selectedOffer: 'Выбранное предложение',
    floatStickersHint: 'Float, стикеры и inspect доступны на странице конкретного лота.',
    fungibleOfferHint: 'Покупка по лучшей цене — без перехода на страницу лота.',
    selectOfferHint: 'Выберите предложение в списке слева, затем нажмите «Купить».',
    lotDetailsLink: 'Подробнее о лоте',
  },
  orderBook: {
    title: 'Стакан заявок',
    subtitle: 'Спрос на покупку и лучшие предложения продажи по этому предмету.',
    loading: 'Загрузка стакана…',
    empty: 'Пока нет заявок на покупку и продажу по этому предмету.',
    spread: 'Спред',
    bidsTitle: 'Покупают',
    asksTitle: 'Продают',
    noBids: 'Пока никто не хочет купить.',
    noAsks: 'Пока никто не продаёт.',
    colPrice: 'Цена',
    colQty: 'Кол-во',
    colFloat: 'Float',
    quantityShort: '{{count}} шт.',
    ownBadge: 'Ваша',
    ownDemandNote: 'Ваша заявка учтена в общем спросе ниже.',
    compactSubtitle: 'Спрос на покупку по этому износу.',
    demandLabel: 'Спрос',
    bestBidLabel: 'Лучший bid',
    sellHint_one: '{{count}} покупатель готов купить до',
    sellHint_few: '{{count}} покупателя готовы купить до',
    sellHint_many: '{{count}} покупателей готовы купить до',
    asksSummary: '{{count}} лотов в продаже',
    fromPrice: 'от',
    openLot: 'Открыть',
    sellHintPrefix: '{{count}} покупателей готовы купить до',
    sellLink: 'Выставить из инвентаря',
  },
  buyRequestPanel: {
    kicker: 'Заявка на покупку',
    howItWorks: 'Как работает заявка',
    lead:
      'Сейчас этот скин никто не продаёт. Оставьте заявку — пришлём уведомление, когда появится подходящее предложение. Средства резервируются на кошельке.',
    activeListTitle: 'Активные заявки',
    activeListSummary: 'Активные заявки ({{count}})',
    active: 'Заявка активна',
    activeBadge: 'Активна',
    upTo: 'До',
    cancel: 'Отменить',
    canceling: 'Отмена…',
    reservedLabel: 'Зарезервировано',
    createdJustNow: 'только что',
    createdMinutesAgo: '{{count}} мин назад',
    createdHoursAgo: '{{count}} ч назад',
    createdDaysAgo: '{{count}} дн назад',
    stateLabel: 'Состояние',
    maxPriceLabel: 'Максимальная цена',
    quantityLabel: 'Количество',
    quantityShort: '{{filled}}/{{total}} шт.',
    reserved: 'зарезервировано',
    reservePreview: 'Будет зарезервировано:',
    depositLink: 'Пополнить кошелёк',
    invalidQuantity: 'Укажите количество от 1 до 99',
    footerPrefix: 'Все заявки — во вкладке',
    footerLink: 'Сделки → Заявки',
  },
  extensionTask: {
    missing: 'Расширение ещё готовит задачу. Обновите страницу через несколько секунд.',
    checkingDelivery: 'Проверяем доставку',
    timeExpired: 'Время истекло',
    retrying: 'Повторяем отправку',
    preparing: 'Готовим обмен',
    itemLabel: 'Предмет:',
    confirmPending: 'Откройте Steam Mobile и подтвердите отправку.',
    confirmPendingHint:
      'Мы не подтверждаем Guard из расширения — только официальное приложение Steam. Страница обновится сама после подтверждения.',
    confirmPendingWaiting: 'Ждём подтверждения в Steam Guard…',
    confirmPendingTimer: 'Ждём подтверждения · {{elapsed}}',
    offerSent: 'Обмен отправлен. Дальше ждём покупателя в Steam.',
    deliveryCheckBody:
      'Предмет уже ушёл из инвентаря. Не создавайте новый offer — проверяем доставку.',
    keepTabOpen: 'Оставьте вкладку Steam открытой под аккаунтом продавца.',
    retryNow: 'Повторить сейчас',
    manualFallbackHint:
      'Если автоотправка не сработала — откройте блок ниже и укажите offer вручную.',
    attemptCount: 'Попытка {{current}} из {{max}}',
  },
  dealHealth: {
    supportCodeLabel: 'Код для поддержки',
    copyDebugPack: 'Скопировать отчёт для поддержки',
    debugCopied: 'Скопировано',
    settlementTitle: 'Сделка на проверке',
    settlementBody: 'Средства защищены. Выплата станет доступна после окна проверки.',
    tradeConfirmedTitle: 'Обмен подтверждён',
    tradeConfirmedBody: 'Платформа сверяет доставку — статус обновится сам.',
    deliveryCheckTitle: 'Проверяем доставку',
    deliveryCheckBody:
      'Предмет уже ушёл из инвентаря. Не отправляйте новый offer — ждём сигнал Steam.',
    guardTitle: 'Нужно подтвердить в Steam Guard',
    guardBody: 'Откройте Steam Mobile и подтвердите отправку. Расширение Guard не подтверждает.',
    manualTitle: 'Нужна ручная отправка',
    manualBody:
      'Автоотправка не завершилась. Откройте Trade URL покупателя, отправьте скин и сохраните ссылку на offer.',
    extensionOfflineTitle: 'Расширение не подключено',
    extensionOfflineBody: 'Подключите расширение ниже — иначе автоотправка не запустится.',
    retryTitle: 'Есть проблема с автоотправкой',
    retryBody: 'Смотрите подсказку и код ниже. Можно повторить или отправить вручную.',
    autoSendTitle: 'Всё ок — отправляем обмен',
    autoSendBody: 'Расширение само отправит trade offer. Оставьте Steam открытым.',
    waitingBuyerTitle: 'Всё ок — ждём покупателя',
    waitingBuyerBody: 'Обмен ушёл. Покупатель должен принять его в Steam.',
    buyerWaitingOfferTitle: 'Всё ок — ждём обмен',
    buyerWaitingOfferBody: 'Продавец отправляет offer. Страница обновится сама.',
    buyerAcceptTitle: 'Ваш шаг — принять в Steam',
    buyerAcceptBody: 'Откройте входящие предложения, проверьте скин и примите обмен.',
    mismatchTitle: 'Не совпадает — не принимайте',
    mismatchBody:
      'Расширение зафиксировало mismatch с заказом. Не принимайте offer в Steam. Ниже — те же проверки, что в overlay.',
    tradeConfirmedSellerTitle: 'Платформа проверяет доставку',
    tradeConfirmedSellerBody:
      'Сверяем обмена Steam и инвентарь. Выплату откроем после подтверждения.',
    tradeConfirmedBuyerTitle: 'Платформа проверяет доставку',
    tradeConfirmedBuyerBody:
      'Предмет должен быть у вас. Ничего нажимать не нужно — статус обновится сам.',
    settlementSellerTitle: 'Средства на проверке (до 8 дней)',
    settlementSellerBody:
      'Защита от chargeback и возврата скина в Steam. Выплата на баланс после окна.',
    settlementBuyerTitle: 'Сделка на проверке',
    settlementBuyerBody:
      'Предмет у вас. Средства продавцу — после проверки площадки / hold.',
    buyerPairTitle: 'Подключите расширение для безопасного принятия обмена',
    buyerPairBody:
      'Offer уже у вас. Расширение покажет щит и сверку скина на странице Steam — до Accept.',
  },
  buyerExtensionPair: {
    eyebrow: 'Безопасный accept',
    promptTitle: 'Подключите расширение для безопасного принятия обмена',
    promptBody:
      'Проверим название, assetId, float и SteamID продавца на странице offer. Расширение не нажимает Accept за вас.',
    readyTitle: 'Расширение готово к безопасному accept',
    readyBody:
      'Откройте проверенный offer — на странице Steam будет щит R.I.P Market. Примите обмен кнопкой Steam.',
    openVerifiedOffer: 'Открыть проверенный offer в Steam',
    connectCta: 'Подключить расширение для безопасного принятия обмена',
    connectSuccess: 'Расширение подключено. Можно открывать offer со щитом R.I.P.',
    panelTitle: 'Расширение для accept',
    notConnected: 'Не подключено — безопасная сверка offer недоступна',
    installTitle: 'Установите расширение для безопасного принятия обмена',
    installBody:
      'Без расширения accept всё ещё возможен в Steam, но без щита и сверки с заказом на странице offer.',
  },
  buyerAcceptWizard: {
    eyebrow: '3 шага до accept',
    title: 'Примите обмен безопасно',
    subtitle:
      'Откройте обмен в Steam, проверьте предмет и продавца, затем примите его. Подтвердить получение можно здесь или в расширении.',
    step1Title: 'Откройте нужный offer',
    step1Body: 'Перейдите по ссылке на конкретный trade offer — не в общий список входящих.',
    step2Title: 'Проверьте щит и скин',
    step2BodyConnected:
      'Проверьте название, износ и продавца в панели R.I.P. При несовпадении не принимайте обмен.',
    step2BodyManual:
      'Сверьте название, float и SteamID продавца с заказом. Подключите расширение — появится щит сверки.',
    step3Title: 'Accept в Steam',
    step3Body:
      'Примите обмен кнопкой Steam. Если предмет уже получен, а заказ ещё ожидает обмена, нажмите «Предмет у меня» здесь или в панели расширения.',
    stateDone: 'Готово',
    stateCurrent: 'Сейчас',
    stateUpcoming: 'Далее',
    stepsSummary: 'Как принять обмен (3 шага)',
    ctaOpenOffer: 'Открыть этот offer в Steam',
    ctaReopenOffer: 'Снова открыть offer для сверки',
    ctaAcceptInSteam: 'Открыть offer и принять в Steam',
    returnHint:
      'Примите обмен в Steam. Если предмет получен, а статус ещё не обновился, подтвердите получение кнопкой «Предмет у меня» здесь.',
    mismatchTitle: 'Не принимайте — mismatch',
    mismatchBody:
      'Offer не совпал с заказом. Не жмите Accept. Откройте поддержку, если нужно разобрать сделку.',
    preAcceptTitle: 'Вижу верное предложение',
    preAcceptBody:
      'Подтвердите, что на странице Steam тот же скин и продавец, что в заказе. Это не заменяет Accept.',
    preAcceptCta: 'Вижу верное предложение',
    preAcceptDone: 'Отметили: предложение верное. Дальше — Accept кнопкой Steam.',
    receivedBody:
      'Уже приняли обмен в Steam и получили предмет? Подтвердите получение здесь, если сайт всё ещё ожидает обмена.',
    receivedCta: 'Предмет у меня',
    receivedConfirmBody:
      'Нажимайте только если нужный предмет уже в вашем инвентаре Steam. Вы подтверждаете получение и разрешаете расчёт по сделке.',
    receivedConfirmCta: 'Подтвердить',
    receivedCancel: 'Отмена',
    receivedDone: 'Получение подтверждено.',
  },
  postAcceptTrust: {
    eyebrow: 'После обмена',
    deliverySellerTitle: 'Платформа проверяет доставку',
    deliverySellerBody:
      'Ждём два сигнала: статус offer в Steam и что скин ушёл из вашего инвентаря.',
    deliverySellerReason: 'Пока идёт проверка — новый offer по этой сделке не нужен.',
    deliveryBuyerTitle: 'Платформа проверяет доставку',
    deliveryBuyerBody:
      'Скин должен уже быть у вас. Мы сверяем обмена Steam и инвентарь — страница обновится сама.',
    deliveryBuyerReason: 'Ничего не оплачивайте в чат и не принимайте другие обмены «вместо» этой сделки.',
    holdSellerTitle: 'Выплата после окна проверки',
    holdSellerBody:
      'Обмен подтверждён. Средства на hold до конца защитного периода.',
    holdSellerReason:
      '8 дней — защита от chargeback и отмены обмена в Steam (trade reversal).',
    holdBuyerTitle: 'Предмет у вас — расчёт у площадки',
    holdBuyerBody:
      'Скин в вашем инвентаре. Продавцу средства станут доступны после проверки / hold.',
    holdBuyerReason: 'Оплата уже на площадке. Не переводите деньги продавцу в Steam-чат.',
    signalOffer: 'Сигнал обмена Steam',
    signalInventory: 'Сигнал инвентаря',
    toneOk: 'Ок',
    tonePending: 'Ждём',
    toneWarn: 'Внимание',
    toneUnknown: 'Уточняем',
    holdUntilLabel: 'Выплата ориентировочно с',
  },
  postTradeReceipt: {
    eyebrow: 'Квитанция',
    bought: 'Купили',
    sold: 'Продали',
    deal: 'Сделка',
    completedHint: 'Сделка успешно завершена.',
    price: 'Цена сделки',
    commission: 'Комиссия площадки (5%)',
    paid: 'Оплатили',
    credited: 'К зачислению',
    amount: 'Сумма',
    offerId: 'обмена Steam',
    walletCta: 'Кошелёк',
    dealsCta: 'Мои сделки',
  },
  tradeEscalation: {
    eyebrow: 'Окно обмена',
    timeLeftMinutes: 'Осталось ~{{minutes}} мин',
    timeLeftHours: 'Осталось ~{{hours}} ч {{minutes}} мин',
    timeExpiredLabel: 'Время на обмен истекло',
    timeoutOk:
      'Если обмен не завершится за это время, откроется автоматический спор — деньги останутся под защитой площадки.',
    timeoutSoon:
      'Меньше 15 минут до автоматического спора. Если оффер не тот или продавец молчит — откройте проблему сейчас.',
    timeoutCritical:
      'Осталось меньше 5 минут. Если что-то пошло не так — нажмите «Проблема с обменом», пока спор не открылся сам.',
    timeoutExpired:
      'Окно обмена закончилось. Если что-то пошло не так — опишите проблему; деньги под защитой площадки.',
    problemCta: 'Проблема с обменом',
    openDisputeCta: 'Открыть спор',
    contactSupportCta: 'Написать в поддержку',
    prefillHint:
      'Мы уже подставили ID сделки, offer и статус проверки — дополните, что случилось.',
  },
  extensionTaskPhase: {
    ACKED: 'Расширение взяло задачу',
    TRADE_PAGE_OPENED: 'Открыли страницу обмена',
    OFFER_DRAFTED: 'Готовим обмен',
    ITEM_SELECTED: 'Добавили предмет',
    OFFER_SUBMITTED: 'Отправляем обмен',
    CONFIRM_PENDING: 'Подтвердите в Steam Guard',
    OFFER_SENT: 'Обмен отправлен',
    OFFER_FAILED: 'Не удалось отправить',
  },
  offerErrorHint: {
    BUYER_TRADE_URL_INVALID: 'Попросите покупателя обновить Trade URL.',
    BUYER_TRADE_URL_MISSING: 'У покупателя нет Trade URL в профиле.',
    ITEM_MISSING:
      'Предмет не найден в Steam. Если обмен уже ушёл — дождитесь проверки доставки, не отправляйте новый offer.',
    ITEM_ALREADY_GONE:
      'Похоже, обмен уже прошёл в Steam. Не отправляйте новый offer — платформа проверяет доставку покупателю.',
    ITEM_MISMATCH:
      'В инвентаре несколько одинаковых скинов — укажите offer вручную или пересоздайте лот.',
    INVENTORY_NOT_LOADED:
      'Не удалось загрузить инвентарь Steam. Откройте steamcommunity.com в этом Chrome и обновите страницу сделки.',
    INVENTORY_PRIVATE:
      'Инвентарь Steam скрыт. Сделайте Inventory: Public в настройках приватности Steam и повторите.',
    INVENTORY_RATE_LIMITED:
      'Steam временно ограничил запросы (429). Подождите 1–2 минуты и нажмите «Повторить».',
    STEAM_COOKIE_EXPIRED:
      'Нет входа в Steam в этом Chrome. Войдите на steamcommunity.com под аккаунтом продавца.',
    STEAM_ACCOUNT_MISMATCH:
      'В Chrome залогинен другой Steam-аккаунт. Войдите под продавцом или подключите расширение в нужном профиле.',
    STEAM_UNAVAILABLE: 'Steam временно недоступен.',
    STEAM_GUARD_REQUIRED: 'Подтвердите обмен в Steam Mobile.',
    CONFIRM_PENDING: 'Ожидается подтверждение в Steam Guard.',
    OFFER_SEND_FAILED: 'Не удалось отправить обмен — попробуйте вручную.',
    OFFER_DRAFT_FAILED: 'Не удалось подготовить обмен в Steam.',
    TRADE_HOLD_BLOCKED:
      'Steam не позволяет отправить обмен из-за trade hold. Подождите снятия ограничения или отправьте вручную.',
    SESSION_REVOKED:
      'Сессия расширения истекла. Откройте «Аккаунт» и нажмите «Подключить расширение» снова.',
    STALE_ORDER_SUPERSEDED:
      'Эта сделка устарела — есть более новый заказ. Откройте актуальную сделку в «Мои сделки».',
    MAX_ATTEMPTS_REACHED:
      'Автоотправка исчерпала попытки. Отправьте offer вручную по ссылке ниже.',
    TASK_TTL_EXPIRED:
      'Время отправки истекло. Откройте заказ: сервис проверит, можно ли безопасно продолжить. Не отправляйте повторный обмен, если результат первой попытки неизвестен.',
  },
  extensionTaskCta: {
    openSteamLogin: 'Войти в нужный Steam',
    openSteamPrivacy: 'Открыть настройки приватности',
    openSteamInventory: 'Открыть инвентарь Steam',
    openAccountRepair: 'Подключить расширение',
    supportCode: 'Код для поддержки',
  },
  extensionUiFlow: {
    uiTrade: 'UI trade (Steam-страница, автозаполнение)',
    apiFallback: 'API fallback (legacy, до rollout)',
  },
  inventory: {
    selectPlaceholderTitle: 'Выберите предмет',
    selectPlaceholderText: 'Кликните по доступному предмету в инвентаре, укажите цену и выставьте лот.',
    sortPriceDesc: 'Сначала дорогие',
    sortPriceAsc: 'Сначала дешёвые',
    sortName: 'По названию',
    showUnavailable: 'Недоступные',
    steamPricesLabel: 'Цены Steam',
    stale: 'Устарело',
    steamRequiredMessage: 'Обновление недоступно: сначала привяжите Steam в настройках аккаунта.',
    steamRequiredBanner: 'Сначала привяжите Steam и укажите Trade URL — без этого инвентарь и обмены недоступны.',
    tradeUrlRequiredPrefix: 'Укажите Trade URL в',
    tradeUrlRequiredLink: 'настройках аккаунта',
    tradeUrlRequiredSuffix: '— без него нельзя выставлять предметы.',
    loadingInventory: 'Загрузка инвентаря…',
    backgroundSync: 'Обновляем инвентарь из Steam в фоне…',
    steamPricesFailed: 'Не удалось загрузить цены Steam.',
    retry: 'Повторить',
    steamPricesMissing: 'Нет цен Steam у {{count}} {{items}} — можно выставить, указав цену вручную.',
    items_one: 'предмета',
    items_few: 'предметов',
    items_many: 'предметов',
    resetDevTradesHint:
      'После тестовых mock-сделок предметы могут остаться в статусе «Продан». Сброс вернёт их в «Доступен» и отменит зависшие сделки.',
    resetDevTradesButton: 'Сбросить тестовые сделки',
    resetDevTradesResetting: 'Сброс…',
    linkSteamFirst: 'Перейти в аккаунт',
  },
  itemOffers: {
    title: 'Предложения продавцов',
    subtitle: 'Выберите конкретный лот, чтобы увидеть float, стикеры и оформить покупку.',
    subtitleSelect: 'Клик по строке выбирает лот для покупки на этой странице.',
    sortAria: 'Сортировка',
    sortPriceAsc: 'Цена ↑',
    sortPriceDesc: 'Цена ↓',
    sortFloatAsc: 'Float ↑',
    sortFloatDesc: 'Float ↓',
    sortNewest: 'Новые',
    loading: 'Загрузка предложений…',
    empty: 'Пока никто не продаёт этот предмет.',
    colPrice: 'Цена',
    colFloat: 'Float',
    colStickers: 'Стикеры',
    colListed: 'Выставлен',
    open: 'Открыть',
    select: 'Выбрать',
    selected: 'Выбрано',
    details: 'Подробнее',
    stickerCount: '{{count}} шт.',
  },
  lotBreadcrumbs: {
    catalog: 'Каталог',
    navAria: 'Навигация по каталогу',
    backToCatalog: 'Назад в каталог',
  },
  lotStickers: {
    label: 'Стикеры',
  },
  myBuyRequests: {
    loading: 'Загрузка заявок…',
    emptyTitle: 'Заявок пока нет',
    emptyMessage: 'Откройте предмет без предложений в каталоге и оставьте заявку на покупку.',
    emptyStep1: 'Найдите предмет в каталоге.',
    emptyStep2: 'Укажите макс. цену и количество.',
    toCatalog: 'Перейти в каталог',
    colItem: 'Предмет',
    colMaxPrice: 'Макс. цена',
    colQuantity: 'Кол-во',
    colReserved: 'Резерв',
    colStatus: 'Статус',
    noLimit: 'Без лимита',
    cancel: 'Отменить',
    colActions: 'Действия',
  },
  buyRequestStatus: {
    OPEN: 'Активна',
    FULFILLED: 'Выполнена',
    CANCELED: 'Отменена',
    EXPIRED: 'Истекла',
  },
  notifications: {
    title: 'Уведомления',
    readAll: 'Прочитать все',
    close: 'Закрыть',
    empty: 'Уведомлений пока нет.',
    viewAll: 'Все уведомления',
    fabAria: 'Уведомления',
    open: 'Открыть',
    toWallet: 'К кошельку',
    unreadCount: '{{count}} непрочитанных',
    allRead: 'Все уведомления прочитаны',
    category: 'Категория',
    eventType: 'Тип события',
    show: 'Показать',
    showAll: 'Все',
    showUnread: 'Только непрочитанные',
    emptyPageTitle: 'Нет уведомлений',
    emptyPageMessage: 'Здесь появятся события по вашим сделкам и кошельку.',
    emptyFilteredTitle: 'Нет уведомлений по фильтру',
    emptyFilteredMessage: 'Сбросьте фильтры — новые события могут быть в другом разделе.',
    emptyBrowseCatalog: 'В каталог',
    resetFilters: 'Сбросить фильтры',
    myDeals: 'Мои сделки',
    categoriesAll: 'Все категории',
    categoryDeals: 'Сделки',
    categoryMoney: 'Деньги',
    categorySystem: 'Система',
    eventsAll: 'Все типы',
    eventsActionRequired: 'Требуют действия',
    eventOrderCreated: 'Создание сделки',
    eventOrderCompleted: 'Завершение',
    eventOrderFailed: 'Неудача',
    eventOrderDispute: 'Спор',
    eventTradeOperation: 'Обмен',
    eventSaleSettled: 'Выплата',
    eventSettlementBlocked: 'Блокировка расчёта',
  },
  notif: {
    ORDER_CREATED: {
      title: 'Сделка оформлена',
      message: 'Средства зарезервированы. Дождитесь trade offer от продавца.',
    },
    ORDER_CREATED_seller: {
      title: 'Новая покупка',
      message: 'Покупатель оплатил лот — отправьте trade offer в Steam.',
    },
    TRADE_OPERATION_CREATED: {
      title: 'Отправьте trade offer',
      message: 'Передайте предмет покупателю в Steam для завершения сделки.',
    },
    TRADE_OPERATION_CREATED_buyer: {
      title: 'Примите обмен в Steam',
      message: 'Продавец готов передать предмет — примите trade offer.',
    },
    ORDER_COMPLETED: {
      title: 'Сделка завершена',
      message: 'Обмен подтверждён, сделка успешно закрыта.',
    },
    ORDER_FAILED: {
      title: 'Сделка не состоялась',
      message: 'Сделка завершилась неуспешно. Средства возвращены при необходимости.',
    },
    ORDER_DISPUTE_OPENED: {
      title: 'Открыт спор',
      message: 'Откройте сделку и следуйте инструкциям поддержки.',
    },
    BUY_REQUEST_MATCHED: {
      title: 'Появилось предложение',
      message:
        'По вашей заявке выставлен новый лот. Успейте купить — предложение может забрать другой.',
    },
    SALE_SETTLED: {
      title: 'Выплата за продажу',
      message: 'Средства за продажу зачислены на кошелёк.',
    },
    SETTLEMENT_BLOCKED: {
      title: 'Расчёт заблокирован',
      message: 'Обмен подтверждён, но выплата заблокирована лимитами политики.',
    },
    TRADE_SHADOW_MISMATCH: {
      title: 'Расхождение trade-статуса',
      message: 'Обнаружено несоответствие статуса обмена. Откройте сделку для проверки.',
    },
    RECONCILIATION_FAILED: {
      title: 'Ошибка сверки ledger',
      message: 'При сверке балансов обнаружены расхождения.',
    },
  },
  readiness: {
    steamRequiredTitle: 'Сначала привяжите Steam',
    steamRequiredBody:
      'Без привязанного Steam-аккаунта покупка недоступна. Перейдите в настройки аккаунта и привяжите Steam, затем вернитесь к оформлению.',
    toAccount: 'Перейти в аккаунт',
    tradeUrlInlinePrefix: 'Trade URL не указан —',
    tradeUrlInlineLink: 'укажите в аккаунте',
    tradeUrlInlineSuffix: 'для покупки.',
    tradeUrlMissingTitle: 'Trade URL не указан',
    tradeUrlMissingBody: 'Без Trade URL покупка недоступна — продавец не сможет отправить обмен в Steam.',
    tradeUrlSetButton: 'Указать Trade URL',
    insufficientTitle: 'Недостаточно средств',
    insufficientNeeded: 'Для покупки нужно минимум {{amount}} на балансе.',
    insufficientGeneric: 'Пополните баланс (USD), чтобы подтвердить покупку.',
    depositButton: 'Пополнить кошелёк',
    tradeHint: 'После покупки вам нужно будет принять trade offer в Steam.',
    steamItemLabel: 'Steam привязан',
    steamHint: 'Привяжите Steam, чтобы синхронизировать инвентарь и участвовать в сделках.',
    tradeUrlItemLabel: 'Ссылка на обмен указана',
    tradeUrlHint: 'Укажите ссылку на обмен в настройках аккаунта для обменов в Steam.',
    steamLinkedLabel: 'Steam привязан',
    tradeUrlReadyLabel: 'Trade URL указан',
    checklistTitle: 'Готовность к сделкам',
  },
  similarLots: {
    title: 'Похожие предложения',
    loading: 'Загрузка похожих лотов…',
    openLotAria: 'Открыть лот {{name}}',
    float: 'Float: {{value}}',
    pattern: 'Паттерн: {{value}}',
    openHint: 'Открыть лот →',
  },
  supportWidget: {
    guest: 'гость',
    title: 'Быстрая помощь',
    close: 'Закрыть',
    greeting: 'Здравствуйте, {{name}}! Чем можем помочь?',
    searchLabel: 'Поиск по частым вопросам',
    searchPlaceholder: 'Поиск по частым вопросам…',
    noResults: 'Ничего не найдено. Напишите нам — поможем вручную.',
    hintSupportPage: 'Нужна помощь с конкретной сделкой? Заполните форму тикета на странице.',
    hintFaqPage: 'Подробные инструкции — в статьях FAQ на этой странице.',
    hintDefault: 'Краткие подсказки здесь. Полная база знаний — в разделе FAQ.',
    emailButton: 'Написать на email',
    ticketFormButton: 'К форме тикета',
    createTicketButton: 'Создать тикет',
    openFaqButton: 'Открыть FAQ',
    fabAria: 'Быстрая помощь',
    navAria: 'Быстрые ссылки',
    navDeals: 'Сделки',
    navWallet: 'Кошелёк',
    navFaq: 'FAQ',
  },
  trustBanner: {
    step1: 'Выбираете предмет',
    step2: 'Деньги в hold',
    step3: 'После передачи сделка завершается',
    title: 'Защищённая сделка',
    subtitle: 'Оплата в hold до подтверждения передачи в Steam.',
    stepsAria: 'Как работает защищённая сделка',
    dismiss: 'Скрыть',
  },
  orderTrade: {
    instruction1: 'Откройте Trade URL покупателя в Steam.',
    instruction2: 'Добавьте предмет из сделки в предложение обмена.',
    instruction3: 'Отправьте trade offer без лишних предметов с вашей стороны.',
    instruction4: 'Скопируйте ссылку на отправленное предложение и вставьте её ниже.',
    instruction5: 'Дождитесь принятия обмена покупателем. Если предмет уже получен, а проверка задерживается, покупатель может подтвердить получение в заказе.',
    checklist1: 'Проверьте название скина и состояние (wear/float).',
    checklist2: 'Убедитесь, что в обмене только ожидаемый предмет.',
    checklist3: 'Не принимайте предложения с лишними предметами от продавца.',
    checklist4:
      'Сверьте SteamID64 отправителя offer с продавцом на этой странице — они должны совпадать.',
    checklist5:
      'Не принимайте обмен, если аккаунт отправителя отличается или выглядит подозрительно.',
  },
  tradePollStatus: {
    accepted: 'Принят',
    steamCheckFailed: 'Сбой проверки Steam',
    declined: 'Отклонён',
    expired: 'Истёк',
    dispute: 'Спор',
    timeout: 'Таймаут',
    checkingSteam: 'Проверяем Steam',
    waiting: 'Ожидание',
  },
  orderTradePanel: {
    yourStep: 'Ваш шаг',
    checkItemTitle: 'Проверьте предмет в Steam',
    checkItemBody:
      'Обмен мог уже пройти. Откройте инвентарь Steam — платформа подтвердит сделку сама.',
    checking: 'Проверяем…',
    checkNow: 'Проверить доставку сейчас',
    awaitingExtension:
      'Продавец отправляет обмен через расширение. Обычно 1–2 минуты — страница обновится сама.',
    awaitingSeller: 'Ждём, пока продавец отправит обмен в Steam.',
    openIncomingOffers: 'Открыть входящие предложения Steam',
    openVerifiedOffer: 'Открыть проверенный offer в Steam',
    checklistSummary: 'Перед принятием проверьте скин',
    ackNotUpdatedSummary: 'Если статус на сайте не обновился',
    ackHint:
      'Эти кнопки не заменяют принятие обмена в Steam — только помогают сайту быстрее сверить статус.',
    saving: 'Сохраняем…',
    seePreAccept: 'Вижу предложение в Steam',
    itemAlreadyReceived: 'Предмет уже у меня в Steam',
    speedUpCheckSummary: 'Ускорить проверку',
    receivedAckPendingSteam:
      'На сайте отметка есть, но скин ещё не виден у вас в Steam. Примите входящий trade offer — статус обновится сам.',
    receivedAck: 'Получение отмечено. Если скин уже в Steam — сделка скоро закроется.',
    extensionAck: 'Ок. Осталось принять обмен в Steam.',
    extensionHintNoAck:
      'С расширением R.I.P Market на странице обмена в Steam будет проверка сделки.',
    confirmGuardTitle: 'Подтвердите в Steam Guard',
    confirmGuardBody:
      'Обмен уже создан. Откройте Steam Mobile и подтвердите отправку — это ваш главный шаг.',
    confirmGuardAutoHint:
      'После подтверждения статус сменится сам — обновлять страницу не нужно.',
    checkingDeliveryTitle: 'Проверяем доставку',
    checkingDeliveryBody:
      'Предмет уже ушёл из вашего инвентаря. Не отправляйте новый обмен — платформа сверяет покупателя.',
    ackSentDone: 'Обмен отмечен как отправленный. Ждём принятия покупателем в Steam.',
    waitingMessage: 'Отправьте trade offer покупателю и сохраните ссылку на предложение ниже.',
    detailsExtraSummary: 'Дополнительно / если автоотправка не сработала',
    detailsManualSummary: 'Куда отправить и как указать обмен',
    manualFallbackTitle: 'Отправьте обмен вручную — это нормально',
    manualFallbackBody:
      'Откройте Trade URL покупателя одним нажатием, добавьте скин из сделки, отправьте offer и вставьте ссылку сюда. Без FAQ.',
    manualFallbackItemLabel: 'Предмет сделки:',
    openTradeUrlPrimary: 'Открыть Trade URL покупателя',
    guardConfirmedHint: 'Если Guard уже подтверждён, а статус на сайте не обновился — отметьте отправку.',
    iSentTrade: 'Я отправил обмен',
    buyerTradeUrlTitle: 'Trade URL покупателя',
    copied: 'Скопировано',
    copy: 'Копировать',
    openInSteam: 'Открыть в Steam',
    buyerTradeUrlMissing:
      'У покупателя не указан Trade URL. Попросите добавить его в настройках аккаунта.',
    instructionsTitle: 'Инструкция',
    tradeOfferInputLabel: 'Ссылка или ID trade offer',
    offerLinkPromptTitle: 'Уже отправили обмен в Steam?',
    offerLinkPromptBody:
      'Если offer уже ушёл (в том числе после Guard), вставьте его ID или ссылку — покупатель увидит «Сделка R.I.P» вместо «не наша сделка».',
    savingTrade: 'Сохраняем…',
    saveOffer: 'Сохранить предложение',
    offerLabel: 'Предложение обмена:',
    pollStatusLabel: 'Статус проверки:',
    scamWarningTitle: 'Проверьте отправителя обмена',
    scamWarningBody:
      'Мошенники могут прислать offer с другого Steam-аккаунта. Перед принятием сверьте SteamID64 отправителя с данными продавца ниже.',
    sellerLabel: 'Продавец',
    buyerLabel: 'Покупатель',
    steamIdLabel: 'SteamID64',
    steamIdMissing: 'SteamID продавца пока недоступен.',
    openSteamProfile: 'Профиль Steam',
    wearLabel: 'Износ',
    floatLabel: 'Float',
    stickersLabel: 'Стикеры',
  },
  tradeUrlBanner: {
    text: 'Укажите Trade URL в Steam — без него нельзя продавать и покупать скины.',
    action: 'Перейти в настройки',
    dismiss: 'Позже',
  },
  oneGesturePair: {
    title: 'Подключите расширение R.I.P — один клик',
    body: 'Расширение уже установлено. Подключите его к аккаунту, чтобы автоотправка обменов и «Продать» в Steam работали.',
    connect: 'Подключить расширение',
    connecting: 'Подключаем…',
    dismiss: 'Позже',
  },
  trc20Address: {
    required: 'Укажите TRC-20 адрес получателя.',
    mustStartWithT: 'TRC-20 адрес должен начинаться с T.',
    invalid: 'Некорректный TRC-20 адрес.',
  },
  account: {
    subtitle: 'Настройки для сделок: Trade URL, Steam и расширение.',
    changeSteamConfirm:
      'Отвязать текущий Steam и привязать другой аккаунт?\n\nСначала откроется выход из Steam в браузере — войдите под нужным аккаунтом, затем завершите привязку на этой странице.',
    changeSteamSuccess:
      'Steam отвязан. Выйдите из Steam в открывшейся вкладке, затем нажмите «Привязать Steam».',
    tradeUrlRequired: 'Укажите Trade URL из Steam.',
    tradeUrlInvalid:
      'Некорректная ссылка. Нужен URL вида https://steamcommunity.com/tradeoffer/new/?partner=…&token=…',
    tradeUrlSaved: 'Ссылка на обмен сохранена.',
    tradeUrlTitle: 'Trade URL',
    tradeUrlNeeded: 'Нужен для обменов в Steam.',
    tradeUrlLabel: 'Ссылка на обмен',
    tradeUrlPlaceholder: 'https://steamcommunity.com/tradeoffer/new/?partner=…&token=…',
    savingTradeUrl: 'Сохранение…',
    saveTradeUrl: 'Сохранить ссылку',
    getTradeUrl: 'Получить ссылку',
    steamSectionTitle: 'Steam',
    linkSteamHint: 'Привяжите Steam для синхронизации инвентаря и сделок.',
    linkSteamRedirecting: 'Перенаправление…',
    linkSteamButton: 'Привязать Steam',
    steamLinkedMessage: 'Steam привязан',
    steamLinkedWithName: 'Steam привязан: {{name}}',
    changeSteamLoading: 'Отвязка…',
    changeSteamButton: 'Сменить Steam',
    steamLinkUnavailable: 'Привязка Steam доступна при AUTH_PROVIDER=steam.',
    name: 'Имя',
    role: 'Роль',
    status: 'Статус',
    steamId: 'Steam ID',
    steamNotLinkedValue: 'Не привязан',
    steamNick: 'Ник Steam',
    steamPersonaLoading: 'Загрузка…',
    devHint: 'Dev: {{provider}}',
  },
  checkout: {
    title: 'Подтверждение покупки',
    backToLot: 'Назад к лоту',
    unavailable: 'Лот недоступен для покупки ({{status}}).',
    toCatalog: 'В каталог',
    loading: 'Загрузка…',
    payLabel: 'К оплате',
    available: 'Доступно',
    shortfall: 'Не хватает',
    hold: 'На hold',
    ownLot: 'Вы не можете купить свой лот.',
    tradeHint: 'После покупки примите trade offer в Steam.',
    depositButton: 'Пополнить кошелёк',
    confirming: 'Создаём сделку…',
    confirmButton: 'Подтвердить покупку',
  },
  createLot: {
    title: 'Установка цены',
    subtitle: 'Укажите цену и проверьте комиссию перед публикацией.',
    back: 'Назад',
    loading: 'Загрузка предмета…',
    notListable: 'Этот предмет сейчас нельзя выставить (не обменивается или в trade-lock).',
    priceHint:
      'Подсказка: в staging популярный тестовый диапазон — $500–$2,000. Комиссия маркетплейса фиксирована: 5%.',
    priceLabel: 'Цена (USD)',
    listPrice: 'Цена лота',
    commission: 'Комиссия (5%)',
    youReceive: 'Вы получите',
    publishing: 'Публикация…',
    publish: 'Выставить лот',
    emptyTitle: 'Предмет не выбран',
    emptyMessage: 'Вернитесь в инвентарь и выберите предмет для продажи.',
    toInventory: 'К инвентарю',
  },
  wallet: {
    minDepositError: 'Минимальное пополнение — {{amount}}.',
    minWithdrawError: 'Минимальный вывод — {{amount}}.',
    warningTokenNetwork: 'Только {{token}} в сети {{network}}.',
    warningOtherLost: 'Другие токены и сети будут потеряны безвозвратно.',
    warningMinDeposit: 'Минимальное пополнение: {{amount}}.',
    warningRate:
      'Баланс маркетплейса в USD. Для этого способа пополнения 1 USDT на адресе = $1.00 на балансе.',
    available: 'Доступно',
    back: 'Назад',
    depositNeeded: 'Для покупки нужно минимум {{amount}} на балансе.',
    whatIsHoldTitle: 'Что такое hold?',
    whatIsHoldBody:
      'При покупке сумма сделки переводится из «Доступно» в «В сделках» — деньги зарезервированы, но ещё не переданы продавцу. После подтверждения обмена в Steam hold списывается в пользу продавца. При отмене или неудачной сделке средства возвращаются в «Доступно».',
    availableHint: 'Можно потратить на покупки',
    hold: 'В сделках',
    holdHint: 'Зарезервировано в активных сделках',
    frozen: 'Заморожено',
    frozenHint: 'Недоступно до завершения проверки или решения спора',
    depositUsdtTitle: 'Пополнить через USDT (TRC-20)',
    depositAwaiting: 'Ожидаем перевод… USD на балансе появится после подтверждений в сети TRON.',
    depositCreditedTitle: 'Пополнение зачислено',
    depositCredited: 'На баланс зачислено {{amount}}.',
    depositHistoryTitle: 'Недавние пополнения',
    realMoneyBetaTitle: 'Закрытая beta: реальные деньги',
    realMoneyBetaBody:
      'Пополнение — через USDT на поддерживаемых сетях (mainnet). На баланс зачисляется USD. Сделки и выплаты продавцам проходят поэтапное включение: средства в hold защищают покупателя до подтверждения обмена в Steam.',
    depositQrAlt: 'QR-код для пополнения USDT TRC-20',
    depositAddressLoading: 'Загрузка адреса…',
    withdrawUsdtTitle: 'Вывод на USDT (TRC-20)',
    withdrawUsdtBody:
      'Сумма в USD спишется с доступного баланса; на указанный адрес уйдёт USDT после проверки.',
    trc20Address: 'TRC-20 адрес',
    commission: 'Комиссия',
    toReceive: 'К получению',
    minimum: 'Минимум',
    historyTitle: 'История выводов',
    receivedAmount: 'к получению {{amount}}',
    testDepositTitle: 'Тестовое пополнение',
    testDepositBody:
      'Зачисляет USD на баланс для проверки покупок на staging. Не настоящие деньги.',
    backToPurchase: 'Вернуться к покупке',
    depositShortfall: 'Не хватает {{amount}}. Пополните кошелёк — и вернитесь к лоту.',
    depositLeadPurchase:
      'Сумма уже подставлена под нехватку. После зачисления нажмите «Вернуться к покупке».',
    depositUnavailable: 'Пополнение временно недоступно. Попробуйте позже или напишите в поддержку.',
    withdrawUnavailable: 'Вывод временно недоступен. Попробуйте позже или напишите в поддержку.',
    withdrawAvailable: 'Доступно к выводу: {{amount}}',
    toCatalog: 'В каталог',
    subtitlePurchase: 'Пополните баланс и вернитесь к покупке.',
    depositLead: 'Выберите способ и сумму. Средства появятся в «Доступно» после подтверждения.',
    depositReturnHint: 'После пополнения вернитесь к покупке.',
    transactionsEmpty: 'История операций пока пуста.',
    transactionsTitle: 'История операций',
    colType: 'Тип',
    colAmount: 'Сумма',
    colDate: 'Дата',
    colOrder: 'Сделка',
    openOrder: 'Открыть сделку',
  },
  walletTabs: {
    deposit: 'Пополнение',
    withdraw: 'Вывод',
    transactions: 'Транзакции',
  },
  ledgerEntry: {
    DEPOSIT: 'Пополнение',
    HOLD_RESERVE: 'Резерв (hold)',
    HOLD_RELEASE: 'Снятие резерва',
    SETTLEMENT_SELLER: 'Выплата продавцу',
    SETTLEMENT_PLATFORM_COMMISSION: 'Комиссия платформы',
    REFUND: 'Возврат средств',
    WITHDRAWAL: 'Вывод средств',
    WITHDRAWAL_REFUND: 'Возврат вывода',
    WITHDRAW: 'Вывод средств',
    WITHDRAW_FEE: 'Комиссия вывода',
    MANUAL_ADJUSTMENT: 'Ручная корректировка',
  },
  withdrawalStatus: {
    PENDING_REVIEW: 'На проверке',
    APPROVED: 'Одобрено',
    PROCESSING: 'В обработке',
    PAID: 'Выплачено',
    REJECTED: 'Отклонено',
    FAILED: 'Ошибка',
  },
  sellerSale: {
    title: 'Как проходит продажа',
    step1: 'Выставляете предмет и указываете цену.',
    step2: 'Покупатель резервирует средства — лот переходит в RESERVED.',
    step3: 'Вы отправляете trade offer в Steam покупателю.',
    step4: 'После подтверждения обмена получаете выплату за вычетом комиссии 5%.',
    mockHint:
      'Staging: исход сделки можно симулировать через mock-trade на странице заказа (доступно покупателю или админу в dev-окружении).',
  },
  inventoryPriceStack: {
    steam: 'Steam',
    onRip: 'На R.I.P.',
    market: 'Маркет',
    noLots: 'нет лотов',
    na: 'н/д',
    from: 'от',
    change7d: '7д',
    change30d: '30д',
  },
  headerWallet: {
    titleWithHold: 'Кошелёк: доступно и средства в hold / заморозке',
    titleAvailable: 'Кошелёк: доступный баланс',
  },
  copyableDealId: {
    copied: 'Скопировано',
    copyIdShort: 'Скопировать ID: {{id}}',
    copiedAria: 'ID сделки скопирован',
    copyIdAria: 'Скопировать ID сделки {{id}}',
    label: 'ID сделки',
    copyButton: 'Скопировать',
    hint: 'Нужен для обращения в поддержку.',
  },
  lotActionButtons: {
    inspectInGame: 'Осмотр в игре',
    inspectLimitedTitle: 'Базовая ссылка — точный предмет может не открыться в CS2',
    inspectUnavailable:
      'Осмотр в игре для этого лота временно недоступен. Проверьте износ на карточке или откройте Steam Market.',
    openSteamMarket: 'Открыть в Steam Маркете',
    openSteamMarketWithName: 'Открыть в Steam Маркете: {{name}}',
  },
  orderItemLink: {
    openDeal: 'Открыть сделку: {{name}}',
    open: 'Открыть {{name}}',
    itemPageHint: 'Страница предмета',
  },
  itemPreview: {
    category: 'Категория',
    wear: 'Wear',
    float: 'Float',
    pattern: 'Pattern',
  },
  itemCompareHeader: {
    eyebrow: '{{count}} предложений в продаже',
  },
  inventoryAssetCardAria: {
    editLotWithCount: 'Изменить лот {{name}}, {{count}} шт.',
    editLot: 'Изменить лот {{name}}',
    selectWithCount: 'Выбрать {{name}}, {{count}} шт.',
    select: 'Выбрать {{name}}',
  },
  catalogLotCard: {
    openAria: 'Открыть лот {{name}}',
    buyAria: 'Купить',
    pattern: 'Паттерн {{value}}',
  },
  login: {
    devOnlyHint: 'Только для QA и локальной разработки (`/login?dev=1`).',
    loggingIn: 'Вход…',
    mockUnavailable: 'Mock login недоступен на этом окружении.',
    toCatalog: 'В каталог',
  },
  steamCallbackPage: {
    incompleteResponse: 'Ответ Steam неполный. Попробуйте войти снова.',
    title: 'Вход через Steam',
    alreadyLinkedHint:
      'Если этот Steam уже привязан к вашему аккаунту, войдите под ним. Если Steam привязан к другому пользователю — используйте тот аккаунт или обратитесь в поддержку.',
    completing: 'Завершаем вход через Steam…',
  },
  supportTicketTopic: {
    deal: 'Проблема со сделкой / обменом',
    deposit: 'Пополнение баланса',
    withdrawal: 'Вывод средств',
    listing: 'Выставление предмета',
    extension: 'Расширение браузера',
    account: 'Аккаунт / Steam / Trade URL',
    other: 'Другое',
  },
  supportPage: {
    ticketCreated: 'Тикет создан. Команда поддержки ответит в этом разделе.',
    openFaq: 'Открыть FAQ',
    createTicket: 'Создать тикет',
    ticketBodyLead: 'Опишите проблему. Если речь о покупке или продаже — вставьте ID сделки',
    ticketBodyLeadContinued:
      '(Сделки → клик по ID в списке или «Скопировать» на странице сделки). Email:',
    loginToCreateTicket: 'Войдите через Steam в шапке сайта, чтобы создать тикет.',
    subjectLabel: 'Тема',
    subjectPlaceholder: 'Выберите тему',
    descriptionLabel: 'Описание',
    descriptionPlaceholder: 'ID сделки (из страницы сделки), что произошло, что уже пробовали…',
    sending: 'Отправка…',
    sendTicket: 'Отправить тикет',
    myTickets: 'Мои тикеты',
    statusOpen: 'Открыт',
    statusResolved: 'Решён',
    supportReplyLabel: 'Ответ поддержки:',
  },
  catalogItemCard: {
    openAria: 'Открыть {{name}}',
    requestAria: '{{name}} — оставить заявку на покупку',
  },
  orders: {
    available: 'Доступно',
    hold: 'В сделках',
    toReceive: 'К получению',
    active: 'Активные',
    awaitingTransfer: 'Ожидают передачи',
    completed: 'Завершены',
    underReview: 'На проверке',
    role: 'Роль',
    status: 'Статус',
    buyer: 'Покупатель',
    seller: 'Продавец',
    all: 'Все',
    toCatalog: 'В каталог',
    toInventory: 'В инвентарь',
    colItem: 'Предмет',
    colRole: 'Роль',
    colAmount: 'Сумма',
    colStatus: 'Статус',
    colDate: 'Дата',
    noFilterResults: 'Нет сделок по выбранным фильтрам.',
    nextStep: 'Дальше:',
  },
  orderPage: {
    title: 'Сделка',
    subtitleSeller: 'Отслеживайте продажу и обмен в Steam.',
    subtitleBuyer: 'Отслеживайте покупку и статус обмена.',
    myDeals: 'Мои сделки',
    loading: 'Загрузка сделки…',
    realSettlementBanner:
      'На этом окружении для вашего Steam-аккаунта включён реальный расчёт после обмена.',
    shadowTradeBannerTitle: 'Проверка обмена (shadow)',
    shadowTradeBannerBody:
      'Статус сделки обновится после подтверждения обмена в Steam. Платформа сверяет offer в фоне — выплата продавцу пока не включается автоматически.',
    liveTradeBannerTitle: 'Реальный обмен в Steam',
    liveTradeBannerBody:
      'Завершите обмен в Steam (продавец отправляет offer, покупатель принимает). Средства в hold до подтверждения доставки. Автовыплата продавцу на этом этапе ещё не включена.',
    itemPageLink: 'Страница предмета',
    lotLink: 'Лот',
    category: 'Категория',
    wear: 'Износ',
    floatLabel: 'Флоат',
    statusHistory: 'История статусов',
    supportLink: 'Написать в поддержку',
    supportLinkSuffix: ' — укажите ID сделки выше.',
    timeoutRemaining: 'Осталось ~{{minutes}} мин. на обмен.',
    timeoutExpired: 'Время на обмен истекло.',
    amount: 'Сумма',
    onHold: 'На hold',
    youReceive: 'Вы получите',
    whatNow: 'Что делать сейчас',
    moreForSupport: 'Подробнее и поддержка',
    role: 'Роль',
    trade: 'Обмен',
    offerId: 'Offer ID',
    deliveryConfirmed: 'Доставка подтверждена — статус заказа обновлён.',
    deliveryCheckDone:
      'Проверка выполнена. Если предмет уже у вас в Steam, подождите ещё немного и повторите.',
    canceling: 'Отменяем…',
    cancelOrder: 'Отменить сделку',
    failedMessage: 'Сделка не состоялась. Средства возвращены при необходимости.',
    disputeMessage:
      'Открыт спор. Закрыть его может только поддержка или админ — опишите ситуацию в тикете, деньги в hold.',
    completedMessage: 'Сделка успешно завершена.',
    canceledMessage: 'Сделка отменена. Средства возвращены при необходимости.',
    extensionTradeModeLabel: 'Режим расширения:',
    devPanelTitle: 'Staging / QA',
    devSimulateHint: 'Dev/stage: смоделировать исход обмена.',
    completing: 'Завершаем…',
    completeTradeMock: 'Завершить обмен (mock)',
    failing: 'Ошибка…',
    failTradeSafe: 'Провалить обмен (safe)',
    failTradeDispute: 'Провалить обмен (спор)',
    timingOut: 'Таймаут…',
    tradeTimeoutDispute: 'Таймаут обмена (спор)',
  },
  orderStatus: {
    CREATED: 'Сделка создана',
    PAYMENT_RESERVED: 'Средства зарезервированы',
    WAITING_TRADE: 'Ждём обмен в Steam',
    TRADE_CONFIRMED: 'Обмен подтверждён',
    SETTLEMENT_HOLD: 'Проверка сделки (до 8 дней)',
    COMPLETED: 'Сделка завершена',
    CANCELED: 'Сделка отменена',
    FAILED: 'Сделка не состоялась',
    DISPUTE: 'Открыт спор',
  },
  orderStatusCompact: {
    CREATED: 'Создана',
    PAYMENT_RESERVED: 'Средства в резерве',
    WAITING_TRADE: 'Обмен в Steam',
    TRADE_CONFIRMED: 'Обмен подтверждён',
    SETTLEMENT_HOLD: 'На проверке',
    COMPLETED: 'Завершена',
    CANCELED: 'Отменена',
    FAILED: 'Не состоялась',
    DISPUTE: 'Спор',
  },
  orderStep: {
    CREATED: 'Создание сделки',
    PAYMENT_RESERVED: 'Резерв средств',
    WAITING_TRADE: 'Обмен в Steam',
    TRADE_CONFIRMED: 'Подтверждение обмена',
    SETTLEMENT_HOLD: 'Период защиты',
    COMPLETED: 'Завершение',
  },
  orderNextAction: {
    completedSeller: {
      title: 'Сделка завершена',
      description: 'Оплата зачислена на ваш баланс (цена лота минус комиссия 5%).',
    },
    completedOther: {
      title: 'Сделка завершена',
      description: 'Средства переведены продавцу. Предмет у вас в инвентаре Steam.',
    },
    settlementHoldSeller: {
      title: 'Средства на проверке',
      description: 'Обмен подтверждён. Выплата на баланс после окна проверки (до 8 дней).',
    },
    settlementHoldOther: {
      title: 'Проверка сделки',
      description: 'Обмен прошёл. Финальный расчёт — после окна проверки.',
    },
    canceled: {
      title: 'Сделка отменена',
      description: 'Зарезервированные средства возвращены на кошелёк.',
    },
    failed: {
      title: 'Сделка не состоялась',
      description: 'Средства возвращены покупателю, лот снова в каталоге.',
    },
    dispute: {
      title: 'Открыт спор',
      description: 'Команда поддержки рассмотрит ситуацию и примет решение.',
    },
    buyerWaitingDeliveryCheck: {
      title: 'Проверьте предмет в Steam',
      description: 'Обмен мог уже пройти. Откройте инвентарь Steam — платформа сверит доставку сама.',
    },
    buyerWaitingNoOffer: {
      title: 'Ждём обмен от продавца',
      description:
        'Продавец отправляет trade offer. Обычно это занимает 1–2 минуты — страница обновится сама.',
    },
    buyerWaitingHasOffer: {
      title: 'Примите обмен в Steam',
      description: 'Откройте входящие предложения, проверьте скин и примите обмен. Сайт обновится сам.',
    },
    buyerTradeConfirmed: {
      title: 'Обмен подтверждён',
      description: 'Платформа завершает проверку. Страница обновится автоматически.',
    },
    buyerSettlementHold: {
      title: 'Проверка сделки',
      description:
        'Обмен подтверждён. Выплата продавцу будет доступна после 8-дневного периода проверки.',
    },
    buyerDefault: {
      title: 'Ожидайте',
      description: 'Сделка обрабатывается. Статус обновится автоматически.',
    },
    sellerWaitingDeliveryCheck: {
      title: 'Проверяем доставку',
      description:
        'Предмет уже ушёл из вашего инвентаря. Не отправляйте новый обмен — платформа сверяет покупателя.',
    },
    sellerWaitingNoOffer: {
      title: 'Ждём автоотправку',
      description:
        'Расширение само отправит обмен. Если Steam попросит — подтвердите в приложении на телефоне.',
    },
    sellerWaitingAwaitingGuard: {
      title: 'Подтвердите в Steam Guard',
      description:
        'Если Steam прислал запрос — подтвердите на телефоне. Дальше ждём принятия покупателем.',
    },
    sellerWaitingAwaitingBuyer: {
      title: 'Ждём покупателя',
      description: 'Обмен ушёл. Покупатель должен принять его во входящих предложениях Steam.',
    },
    sellerTradeConfirmed: {
      title: 'Обмен подтверждён',
      description: 'Ожидаем выплату на ваш кошелёк.',
    },
    sellerSettlementHold: {
      title: 'Проверка сделки',
      description: 'Сделка подтверждена. Средства будут зачислены после окончания 8-дневной проверки.',
    },
    sellerDefault: {
      title: 'Ожидайте',
      description: 'Сделка обрабатывается.',
    },
  },
  apiError: {
    VALIDATION_ERROR: 'Проверьте форму и повторите попытку.',
    UNAUTHORIZED: 'Сессия устарела. Выйдите и войдите снова.',
    FORBIDDEN: 'У вас нет прав на это действие.',
    NOT_FOUND: 'Запрашиваемый объект не найден.',
    BAD_REQUEST: 'Сейчас это действие недоступно.',
    INTERNAL_ERROR: 'Внутренняя ошибка сервера. Попробуйте позже.',
    INVENTORY_ASSET_NOT_AVAILABLE: 'Предмет недоступен для выставления.',
    INVENTORY_ASSET_NOT_TRADABLE: 'Предмет сейчас нельзя обменять.',
    INVENTORY_ASSET_TRADE_LOCKED: 'Предмет в trade-lock.',
    INVENTORY_ASSET_NOT_FOUND: 'Предмет не найден в инвентаре.',
    LOT_ALREADY_EXISTS_FOR_ASSET: 'Для этого предмета уже есть активный лот.',
    SELLER_NOT_ACTIVE: 'Аккаунт продавца не активен.',
    INSUFFICIENT_BALANCE: 'Недостаточно средств. Пополните кошелёк.',
    LOT_NOT_ACTIVE: 'Лот больше недоступен для покупки.',
    LOT_NOT_FOUND: 'Лот не найден.',
    CANNOT_BUY_OWN_LOT: 'Нельзя купить свой собственный лот.',
    LOT_HAS_OPEN_ORDER: 'Этот лот уже покупают.',
    BUYER_NOT_ACTIVE: 'Аккаунт покупателя не активен.',
    ORDER_NOT_FOUND: 'Сделка не найдена.',
    IDEMPOTENCY_KEY_REQUIRED: 'Не удалось выполнить запрос. Обновите страницу и повторите.',
    STEAM_AUTH_FAILED:
      'Не удалось войти через Steam. Если ошибка повторяется — Steam может временно блокировать сервер; используйте Mock-вход или попробуйте позже.',
    STEAM_ALREADY_LINKED:
      'Этот Steam-аккаунт уже привязан к другому пользователю. Войдите в аккаунт, где Steam уже привязан.',
    STEAM_NOT_LINKED: 'Привяжите Steam-аккаунт на странице «Аккаунт» перед синхронизацией инвентаря.',
    STEAM_PROFILE_PRIVATE: 'Инвентарь Steam скрыт. Откройте его в настройках приватности Steam.',
    STEAM_BLOCKED:
      'Steam временно блокирует запросы с сервера. Это не настройки приватности — кэш инвентаря доступен; цены и обновление могут не работать.',
    STEAM_RATE_LIMITED:
      'Steam ограничивает частоту запросов с сервера. Подождите 1–2 минуты и обновите, либо выставьте предмет из инвентаря Steam через расширение.',
    STEAM_ACCOUNT_CHANGED:
      'Steam-аккаунт сменился во время синхронизации. Обновите инвентарь ещё раз.',
    INVENTORY_STALE: 'Не удалось обновить инвентарь из Steam. Попробуйте чуть позже.',
    TRADE_URL_REQUIRED: 'Укажите Trade URL в настройках аккаунта — без него нельзя продавать и покупать.',
    TRADE_URL_STEAM_MISMATCH:
      'Trade URL не принадлежит привязанному Steam-аккаунту. Скопируйте ссылку из настроек Steam того же профиля.',
    STEAM_VAC_BANNED: 'Аккаунт с VAC-баном не может торговать на площадке.',
    STEAM_GAME_BANNED:
      'Аккаунт с игровым баном Steam не может торговать на площадке.',
    STEAM_BAN_CHECK_UNAVAILABLE:
      'Не удалось проверить статус банов Steam — это не бан. Повторите через минуту.',
  },
  userRole: {
    BUYER: 'Покупатель',
    SELLER: 'Продавец',
    ADMIN: 'Админ',
  },
  userStatus: {
    ACTIVE: 'Активен',
    SUSPENDED: 'Приостановлен',
    BANNED: 'Заблокирован',
  },
  tradeStatus: {
    WAITING: 'Ожидание обмена',
    CONFIRMED: 'Обмен подтверждён',
    FAILED_SAFE: 'Неудача (возврат)',
    FAILED_DISPUTE: 'Неудача (спор)',
    TIMEOUT: 'Истекло время',
  },
  steamCallback: {
    genericFailure: 'Не удалось завершить вход через Steam.',
    actionHome: 'На главную',
    actionAccount: 'Открыть аккаунт',
    actionCatalog: 'Вернуться в каталог',
  },
} as const;
