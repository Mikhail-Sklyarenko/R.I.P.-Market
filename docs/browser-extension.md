...
**Deal Shield (1a–1e, v0.6.38):** unified trust card on Steam offer page, tradeoffers detail, popup, and order page — counterparty **avatar + role label + SteamID + profile link**, item wear/float/stickers (honest: hide if unknown), partner SteamID compare vs DOM/URL (mismatch blocks Accept assist), seller **pre-send** banner on `/tradeoffer/new`. Backend `observedPartnerSteamId` on verify.
