...
it("rejects lookalike origins and unrelated paths even with matching partner/token", () => {
  const buyer = "https://steamcommunity.com/tradeoffer/new/?partner=1&token=a";
  expect(
    isTabOnBuyerTradeUrl(
      buyer.replace("steamcommunity.com", "example.com"),
      buyer,
    ),
  ).toBe(false);
  expect(
    isTabOnBuyerTradeUrl(
      buyer.replace("/tradeoffer/new/", "/profiles/1/"),
      buyer,
    ),
  ).toBe(false);
  expect(
    isConcreteSteamTradeOfferUrl("https://example.com/tradeoffer/123456789/"),
  ).toBe(false);
});
